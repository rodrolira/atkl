...
const Cloudinary = () => {
    ...
    const preset_name = "atklrecords";                         //16 Pegamos el "name" rescatado en el punto 24
    const cloud_name = "dotfwyxwr"                          //16.2 Pegamos el cloud_name rescatado en punto 20

    const [ image, setImage ] = useState('');       //12 Creamos estado local que guarde la url de la imagen subida
    const [ loading, setLoading ] = useState(false) //7 Creamos un estado local con valor inicial boolean "false" para saber si la imagen esta cargando.
    ...
    const uploadImage = async (e)=>{            //2 Preparamos para recibir el evento al ejecutarse la función async
        ...
        data.append('file', files[0])           //5 Utilizando método append() agregamos al data el archivo desde files[0]
        ...
        try {
            ...
            setLoading(false);                      //14 Dejamos el loading en false para que intente mostrar la imagen
