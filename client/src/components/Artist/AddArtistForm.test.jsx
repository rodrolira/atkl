...
import AddArtistForm from './AddArtist/AddArtistForm';
...
describe('AddArtistForm', () => {
  ...
  it('validates the form fields', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('addArtist.validation.artistNameRequired'),
      ).toBeInTheDocument();
      expect(
        screen.getByText('addArtist.validation.usernameRequired'),
      ).toBeInTheDocument();
      expect(
        screen.getByText('addArtist.validation.emailRequired'),
      ).toBeInTheDocument();
      expect(
        screen.getByText('addArtist.validation.passwordRequired'),
      ).toBeInTheDocument();
    });
  });
  ...
  it('calls createArtist on form submission', async () => {
    ...
    const useArtistsMock = jest.spyOn(
      require('../../contexts/ArtistContext'),
      'useArtists',
    );
    ...
    fireEvent.change(screen.getByLabelText('addArtist.label.artist_name'), {
      target: { value: 'Test Artist' },
    });
    fireEvent.change(screen.getByLabelText('addArtist.label.username'), {
      target: { value: 'testuser' },
    });
    fireEvent.change(screen.getByLabelText('addArtist.label.email'), {
      target: { value: 'test@example.com' },
    });
    fireEvent.change(screen.getByLabelText('addArtist.label.password'), {
